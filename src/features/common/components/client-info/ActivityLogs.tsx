...
import type { ClientInfoTaskLogResponse } from "../../../../types/client";
...
> = {
  ...
  RESUBMITTED: { label: "Resubmitted", icon: RotateCw, color: "text-blue-600", bg: "bg-blue-100" },
...
function LogEntry({
  ...
  const fetchComment = useCallback(
    async () => {
      const res = await clientAPI.getClientInfoLogComment(taskId, log.id);
      return res.comment;
    },
    [taskId, log.id],
  );
...
export default function ActivityLogs({
  ...
  const [logs, setLogs] = useState<ClientInfoTaskLogResponse[]>([]);
