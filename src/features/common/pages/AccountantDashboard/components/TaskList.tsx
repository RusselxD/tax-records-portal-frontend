...
const accentStyles: Record<
  AccentColor,
  { border: string; badge: string; badgeText: string; iconBg: string }
> = {
  navy: {
...
export default function TaskList({
    ...
    (task: TaskItem): CardField[] => {
      ...
      if (showStatus && task.status) {
        fields.splice(2, 0, {
          ...
          value: (
            <span
              className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium ${statusStyles[task.status]}`}
            >
              <span
                className={`h-1.5 w-1.5 rounded-full ${statusDotColors[task.status]}`}
              />
  ...
  const secondaryFields = useCallback(
    (task: TaskItem): CardField[] => [
      {
        label: "Category",
        value: `${task.categoryName} › ${task.subCategoryName}`,
      },
      {
        label: "Period",
      ...
      {
        label: "Created By",
        value: `${task.createdBy} · ${formatDate(task.createdAt)}`,
      },
    ],
    [],
  );

  const cardClassName = useCallback((task: TaskItem) => {
  ...
  return (
    <div
      className={`bg-white rounded-lg custom-shadow border border-gray-200 border-l-[3px] ${styles.border} overflow-hidden`}
    >
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
      ...
      >
        <span
          className={`flex items-center justify-center w-8 h-8 rounded-lg shrink-0 ${styles.iconBg}`}
        >
          {icon}
        </span>
        ...
        {totalElements > 0 ? (
          <span
            className={`inline-flex items-center justify-center min-w-[24px] h-6 px-2 rounded-full text-xs font-semibold ${styles.badge} ${styles.badgeText}`}
          >
            {formatNum(totalElements)}
          </span>
        ) : (
                ...
                <table className="w-full table-fixed">
                  <thead>
                    <tr className="border-b border-gray-200">
                      <th
                        className={`th-label ${showStatus ? "w-[22%]" : "w-[22%]"}`}
                      >
                        Client Name
                      </th>
                      <th
                        className={`th-label ${showStatus ? "w-[25%]" : "w-[30%]"}`}
                      >
                        Task
                      </th>
                      <th
                        className={`th-label ${showStatus ? "w-[8%]" : "w-[10%]"}`}
                      >
                        Period
                      </th>
                      {showStatus && (
                      ...
                      <th
                        className={`th-label ${showStatus ? "w-[12%]" : "w-[14%]"}`}
                      >
                        Deadline
                      </th>
                      <th
                        className={`th-label ${showStatus ? "w-[16%]" : "w-[18%]"}`}
                      >
                        Created
                      </th>
                    </tr>
                  ...
                  <tbody>
                    {tasks.length === 0 ? (
                      ...
                      tasks.map((task) => {
                        const isOverdue = task.isOverdue ?? false;
                        const isRejected =
                          task.status === TAX_RECORD_TASK_STATUS.REJECTED;
